...
class CurrentDependantCurrentSource(Branch):
    ...
    def get_aux_eq(self) -> Equation:
        return self.branch_of_current.get_current_eq(self.current_out_of) + Equation({(self, self.nodes[0]):-1})
...
class TensionSource(Branch):
    def __init__(self, value:float, n_plus:'Node', n_minus:'Node', name:str=""):
        super().__init__([n_plus, n_minus], name=name)
...
class IndependantTensionSource(TensionSource):
    def __init__(self, value:float, n_plus:'Node', n_minus:'Node', name:str=""):
        super().__init__(value, n_plus, n_minus, name=name)
...
class CurrentDependantTensionSource(TensionSource):
    def __init__(self, multiplier:float, n_plus:Node, n_minus:Node, branch_of_current: Branch, current_out_of: Node, name:str=""):
        super().__init__(multiplier, n_plus, n_minus, name=name)
        ...
        if n_plus.solved and n_minus.solved and (branch_of_current.i is not None) and (n_plus.v - n_minus.v != (branch_of_current.i if branch_of_current.nodes[0] == current_out_of else -branch_of_current.i)):
            raise Exception(f"Impossible circuit!\n{n_plus.name} and {n_minus.name} cant have a CurrentDependantTensionSource of {self.multiplier}.")
        
    def get_aux_eq(self) -> Equation:
        return (self.branch_of_current.get_current_eq(self.current_out_of) * -self.multiplier) + Equation({ self.n_plus:1, self.n_minus:-1})

class TensionDependantTensionSource(TensionSource):
    def __init__(self, multiplier:float, n_plus:Node, n_minus:Node, dep_n_plus:Node, dep_n_minus:Node, name:str=""):
        super().__init__(multiplier, n_plus, n_minus, name=name)
        self.multiplier:float = multiplier
